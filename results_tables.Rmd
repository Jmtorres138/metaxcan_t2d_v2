...
plot_table <- function(table.df,outname,start,stop,rep=FALSE,lws=FALSE){
  ...
  plot.df <- dplyr::select(table.df,
           -one_of("gene.start","gene.end",
                   "total.replicated","gene.type","fg.sig","fi.sig"))
  if(lws==TRUE){
  ...
  rep.vec <- c(1:dim(table.df)[1])[table.df$gene.type=="reported"]
  reg.vec <- c(1:dim(table.df)[1])[table.df$gene.type=="t2d.region"]
  nov.vec <- c(1:dim(table.df)[1])[table.df$gene.type=="novel.locus"]
  fg.vec <- c(1:dim(table.df)[1])[table.df$fg.sig]
  fi.vec <- c(1:dim(table.df)[1])[table.df$fi.sig]

  if(rep==TRUE){
    tabgrob <- edit_cell(tabgrob, row=fi.vec, col=c(1,2,4,14,15), "core-fg", 
                   gp=gpar(fontface="bold"))    
    tabgrob <- edit_cell(tabgrob, row=fg.vec, col=c(1,2,4,12,13), "core-fg", 
                   gp=gpar(fontface="bold")) 
    tabgrob <- edit_cell(tabgrob, row=fg.vec, col=2, "core-fg", 
                   gp=gpar(fontface="bold.italic")) 
    tabgrob <- edit_cell(tabgrob, row=fi.vec, col=2, "core-fg", 
                   gp=gpar(fontface="bold.italic")) 
    ...
    tabgrob <- edit_cell(tabgrob, row=vec, col=c(8,9), "core-bg", 
                   gp=gpar(fill="grey",alpha=0.5, 
                           col = "grey", lwd=1)) 
    tabgrob <- edit_cell(tabgrob, row=rep.vec, col=c(1:5), "core-bg", 
                   gp=gpar(fill=viridis(2)[1],alpha=0.5, 
                           col =viridis(2)[1], lwd=1))  
    tabgrob <- edit_cell(tabgrob, row=reg.vec, col=c(1:5), "core-bg", 
                   gp=gpar(fill=viridis(2)[2],alpha=0.5, 
                           col =viridis(2)[2], lwd=1))
    tabgrob <- edit_cell(tabgrob, row=nov.vec, col=c(1:5), "core-bg", 
                   gp=gpar(fill="grey",alpha=0.5, 
                           col ="grey", lwd=1))
  }
...
table.df <- build_table_df(scott.df,scott.islet)
table.df <- append_magic_info(table.df,fg.sig.gtex,data.frame(),
...
table.df <- build_table_df(scott.df,scott.islet) # 166 rows total 
table.df <- append_magic_info(table.df,fg.sig.gtex,fi.sig.gtex,
