...
gwas_manhat <- function(gwas.df){
  ...
  map.df$y <- sample(seq(0.2,0.8,0.1),dim(map.df)[1],replace=TRUE)
  l[[1]] + geom_segment(data=map.df,aes(x=start,xend=end),y=-1,yend=0,
                        color="black") +
    geom_text(data=map.df,aes(x=(start+end)/2,label=Region,y=y),
                    color="black",size=1.5,segment.alpha = 0.4,segment.size=0.2)
}
