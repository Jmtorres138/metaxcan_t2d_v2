...
Prepare GWAS data frame 

```{r}


build_gwas_df <- function(){
  gwas.df <- fread(df.dir %&% "DIAGRAM_T2D_SCOTT.ma")
  names(gwas.df)[names(gwas.df)=="p"] <- "P"
  names(gwas.df)[names(gwas.df)=="SNP"] <- "RSID"
  snp.df <- fread("cat " %&% df.dir %&% "snp_keyfile.txt.gz" %&% " | zmore")
  ...
  for (i in 1:dim(gwas.df)[1]){
    ...
    row.df <- gwas.df[i,]
    sub.df <- filter(snp.df,RSID==row.df$RSID)
    if(dim(sub.df)[1]==0){
      sub.df <- data.frame(CHR=NA,POS=NA)
    }
    build.df <- cbind(row.df,sub.df) #inner_join(row.df,sub.df,by="RSID") %>% suppressMessages(.)
...
#gwas.df <- build_gwas_df() # Computationally intensive - run on Rescomp 
